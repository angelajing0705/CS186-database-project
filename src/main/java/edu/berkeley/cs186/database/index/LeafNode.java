...
class LeafNode extends BPlusNode {
    ...
    public void remove(DataBox key) {
        // TODO(proj2): implement
        int correct_index = this.keys.indexOf(key);
        if (correct_index != -1) {
            this.keys.remove(correct_index);
            this.rids.remove(correct_index);
        }
        sync();
        return;
    }
